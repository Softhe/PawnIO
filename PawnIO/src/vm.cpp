...
#ifdef PAWNIO_TRUST_CUSTOM
#include <custom_trusted_key.h>
#endif

constexpr static trusted_pubkey k_trusted_keys[] = {
#ifdef PAWNIO_TRUST_UPSTREAM
  {k_pubkey_namazso_2023, sizeof(k_pubkey_namazso_2023)},
#endif
#ifdef PAWNIO_TRUST_CUSTOM
  {k_pubkey_custom, sizeof(k_pubkey_custom)},
#endif
  {}
};
...
static NTSTATUS vm_load_binary_internal(context** ctx, PVOID buffer, SIZE_T size) {
  *ctx = nullptr;
  ...
  ULONG sig_len{};
  memcpy(&sig_len, buffer, sizeof(sig_len));
  if (sig_len > (size - 4))
    return STATUS_INVALID_PARAMETER;
  ...
  auto status = module_trust_verify(mem, len, (uint8_t*)buffer + 4, sig_len);
